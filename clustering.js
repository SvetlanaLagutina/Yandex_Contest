...
module.exports = function(points, x0, y0, zoom) {
  const size = 200 / zoom;
  const minX = x0 - size/2;
  const maxX = x0 + size/2;
  const minY = y0 - size/2;
  const maxY = y0 + size/2;
  ...
  for (let point of points) {
    if (point.x >= minX && point.x <= maxX && point.y >= minY && point.y <= maxY) {
      const i = Math.floor((point.x - minX) / size * 10);
      const j = Math.floor((point.y - minY) / size * 10);
      const index = j * 10 + i;
      cells[index].count++;
    }
  }
